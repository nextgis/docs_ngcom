...
.. note::
    В облаке описываемая в данном разделе функциональность доступна на всех тарифных планах. Но добавлять новых пользователей вы сможете только на плане `Премиум <https://nextgis.ru/pricing-base/>`_. На остальных доступен только один сборщик.

   Для Веб ГИС, развёрнутых на своём сервере, модуль для сбора данных доступен в NextGIS Web `Extended <https://nextgis.ru/pricing/#ngwextended>`_ и `Enterprise <https://nextgis.ru/pricing/#ngwenterprise>`_.
...
1. Организатор сбора регистрируется на nextgis.com_ и создает Веб ГИС.
...
- Организатор сбора данных должен иметь Веб ГИС.
...
Отройте Веб ГИС и создайте в ней следующие ресурсы:

1. `Пустой векторный слой <https://docs.nextgis.ru/docs_ngweb/source/layers.html#ngw-create-empty-vector-layer>`_. 

2. Внутри него - `форму сбора данных <https://docs.nextgis.ru/docs_ngweb/source/collector.html#collector-create-form>`_. 

3. `Подложку <https://docs.nextgis.ru/docs_ngweb/source/webmaps_admin.html#ngw-create-basemap>`_, если сборщику на мобильном устройстве нужно будет видеть карту.

4. Если вы хотите отображать собранные данные на веб-карте, то создайте ресурс `веб-карта <https://docs.nextgis.ru/docs_ngweb/source/webmaps_admin.html>`_ и добавьте на него слой, в который будут собираться данные.

5. Создайте проект сбора данных. Выберите в панели «Создать ресурс» ссылку «Проект Collector»:
...
Ввести наименование проекта. Это наименование будет доступно в мобильном приложении `NextGIS Collector`_:
...
Следующий этап - добавление необходимых элементов в проект.
...
3. Создать Веб ГИС на nextgis.com, убедиться, что подключен сбор данных.
