...
.. important::
    Этот пользователь не должен быть одновременно включён в группу Администраторы и Редакторы. Включайте его только в группу Администраторы.   
...
Чек-лист для администратора - минимальный случай
------------------------------------------------
Это минимальная инструкция для администратора Веб ГИС по организации сбора полевых данных через NextGIS Collector. Это список операций, которые нужно выполнить администратору.

1. Сформулировать перечень параметров объекта, данные по которому будут собираться.
2. Зарегистрировать участников сбора на my.nextgis.com, собрать email по которыми они зарегистрировались.
3. Установить локально текущую версию настольного ПО NextGIS (в частности NextGIS Formbuilder).
4. Создать Веб ГИС на nextgis.com, убедиться, что подключен сбор данных (Премиум).
5. Запустить Formbuilder. Создать новую форму.
6. В Formbuilder отправить форму в Веб ГИС. Запомнить как называется слой и куда вы его положили.
7. Зайти в ☰ - Панель управления, Настройки, Проект Collector, добавить участников сбора по их email.
6. В Веб ГИС создать проект Collector. Ввести название. Вкладка Проект - Имя Пользователя - administrator, ввести пароль. 
8. Далее, вкладка Элементы, добавить слой созданный через Formbuilder (в меню выбрать Cлой).
9. Далее, вкладка Участники, поставить галки напротив тех участников кто будет участвовать в данном проекте. 
10. Сохранить проект.
11. Участники могут запускать NextGIS Collector и после входа они смогут подключиться к сбору в собранном для них проекте.


Чек-лист для администратора - расширенный случай
------------------------------------------------

Это сокращённая инструкция для администратора Веб ГИС по организации сбора полевых данных через NextGIS Collector. Это список операций, которые нужно выполнить администратору.
...
4. Запустить Formbuilder. Создать новую форму. Задать псевдонимы полей. Задать дефолтные варианты в списках.
5. В Formbuilder отправить форму в Веб ГИС. Эта операция создаст векторный слой с формой и векторным стилем.
6. В Веб ГИС создать аккаунт с соответствующими правами, от имени которого будут добавляться записи в Веб ГИС. Например:
   Полное имя: Collector. Логин: Collector. Пароль. Описание: Аккаунт для работы NextGIS Collector. Член Групп - только Administrator.
7. В Веб ГИС создать подложку, использовать запрос OpenStreetMap. Выбрать OpenStreetMap Standard aka Mapnik.
8. В Веб ГИС создать проект сбора. Ввести название. Проект - Имя Пользователя - Collector, ввести пароль. 
9. Добавить в проект сбора слой созданный через Formbuilder (в меню выбрать Cлой). Добавить подложку. Сохранить.
10. Добавить себя как участника. ☰ - Панель управления - Проекты Collector. Ввести e-mail, зарегистрированый на nextgis.com
11. Проверить форму на своём устройстве. 
12. Добавить остальных участников. Проще будет, если они будут регистрироваться на свой аккаунт в gmail. Его клиент всегда есть на смартфоне, и уже залогиненый. Ввести имя и фамилии сборщиков.
