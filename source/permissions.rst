...
.. note::
    Описываемая в данном разделе функциональность доступна в Веб ГИС, находящейся на тарифном плане `Premium <https://nextgis.ru/pricing-base/>`_ или развёрнута `на своём сервере <http://nextgis.ru/pricing/>`_.
    В Веб ГИС на плане Free все ресурсы доступны для чтения. На плане Premium и в Веб ГИС, развёрнутой на своём сервере, по умолчанию все ресуры закрыты от всех, кроме администратора Веб ГИС. Подробне см. правило 2 ниже.
...
По умолчанию владелец Веб ГИС и пользователи, добавленные в группу Администраторы, имеют право просматривать права всех пользователей. Это право относится к глобальным и настраивается `в панели управления <https://docs.nextgis.ru/docs_ngweb/source/users.html#ngw-group-rights>`_.
...
2. Для группы ресурсов, где лежат данные для веб-карты **Ресурс: Чтение, Данные: Чтение данных**, применить для **Этого и вложенных ресурсов**:

.. figure:: _static/allow_guest_data_group_ru_3.png
...
2. Для группы ресурсов, где лежат данные для веб-карты и слой PostGIS **Ресурс: Чтение, Данные: Чтение данных** и **Внешние соединения: Использование соединения**. Применить для **Этого и вложенных ресурсов**:
...
3. Для группы ресурсов, где лежит веб-карта, если это не та же самая группа, где расположены данные, также нужно задать разрешение **Ресурс: Чтение**. Применить **Только для этого ресурса**:
...
4. Для веб-карты **Ресурс: Чтение**. Применить **Только для этого ресурса**.
...
2. Права: **Ресурс: Чтение** и **Данные: Чтение данных** для этого и вложенных ресурсов
