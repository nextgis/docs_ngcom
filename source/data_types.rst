...
* Vector - :term:`ESRI Shape`, :term:`GeoJSON`, :term:`GeoPackage`, :term:`GML`, :term:`KML`, :term:`CSV` or XLSX formats are supported. For :term:`CSV` and :term:`XLSX` only points are supported, coordinates must be put in lat and lot columns.
...
You can add images (including photos) to vector geodata from local and external sources. Image files should be in JPEG or PNG format. Read more `here <https://docs.nextgis.com/docs_ngweb/source/feature_edit.html#ngw-add-photos/>`_.
