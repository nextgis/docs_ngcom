...
* `Соединение PostGIS <https://docs.nextgis.ru/docs_ngweb/source/layers.html#ngw-create-postgis>`_ (создается для подключения к внешней базе данных :term:`PostGIS`)
* `Слой PostGIS <https://docs.nextgis.ru/docs_ngweb/source/layers.html#ngw-create-postgis-layer>`_ (создается для подключения к конкретной таблице с геоданными через существующее Соединение PostGIS)
* `Соединение WMS <https://docs.nextgis.ru/docs_ngweb/source/layers.html#c-wms>`_ (создается для подключения к внешнему серверу :term:`WMS`)
